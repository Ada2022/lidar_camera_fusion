...
def generate_transition_cov(flag, pos_cov = 0.05, vel_cov = 5, dim_cov = 0.1):
    if flag == 0:
        transition_covariance = 0.3 * np.eye(4) #[n_dim_state, n_dim_state] array
    elif flag == 1:
...
def generate_observation_cov(flag, pos_cov = 5, dim_cov = 0.5):
    if flag == 0:
        observation_covariance = 0.3 * np.eye(4) #[n_dim_obs, n_dim_obs] 
    elif flag == 1:
...
#### kalman filter init
def kalman_filter_init(state_dim, ob_dim, initial_state_covariance = None, transition_covariance = None, observation_covariance = None):
    transition_matrix = np.eye(state_dim) #[n_dim_state, n_dim_state] array
    if state_dim == 6:
        transition_matrix[0][4] = 1e3
        transition_matrix[1][5] = 1e3
    ...
    #observation_covariance = 0.3 * np.eye(ob_dim) #[n_dim_obs, n_dim_obs]
...
transition_covariance_person = generate_transition_cov(1)
observation_covariance_person = generate_observation_cov(1)
transition_covariance_pier = generate_transition_cov(0)
observation_covariance_pier = generate_observation_cov(0)
pier_ekf = kalman_filter_init(4, 4, transition_covariance_pier, transition_covariance_pier, observation_covariance_pier)
person_ekf = kalman_filter_init(6, 4, transition_covariance_person, transition_covariance_person, observation_covariance_person)
...
#         col_idx.append(c)
#     return row_idx, col_idx
def update_person_track(tracked, cur, kf):
    '''
    tracked: List of status, cov, t
    cur: List of t, pos
    '''
    ...
    for i in range(len(row_idx)):
        if row_idx[i] > len(tracked) - 1: # tracked_obj less than cur(some apeared from scene)
            ...
            status = list(obs)
            status.extend([0,0]) # vel
            tracked.append([np.array(status).flatten(), kf.transition_covariance, ob_time]) # pos, cov,vel, t
            ...
            (last_status, last_cov, last_time) = tracked[row_idx[i]]
            (ob_time, obs) = cur[col_idx[i]]
            
            trans_matrix = kf.transition_matrices
            trans_matrix[0][4] =  (ob_time - last_time)
            trans_matrix[1][5] =  (ob_time - last_time)


            print("last_status", last_status)
            status, cov = kf.filter_update(last_status, last_cov, obs, transition_matrix = trans_matrix)

            tracked[row_idx[i]][0] = status
    ...
    for i in range(len(row_idx)):
        if row_idx[i] > len(tracked) - 1: # tracked_obj less than cur(some apeared from scene)
            ...
            if trans_matrix.shape[0] > 4:
                trans_matrix[0][4] = ob_time - last_time
                trans_matrix[1][5] = ob_time - last_time
                pos, cov = kf.filter_update(last_pos, last_cov, obs, transition_matrix = trans_matrix)
            else:
                pos, cov = kf.filter_update(last_pos, last_cov, obs)
...
def sensorFusionCallback(image, camera_info, velodyne,  yolo_client,  lidar_client, obstacle_pub):
    ...
    if len(tracked_piers) == 0: 
        for t, p in cur_piers:
            tracked_piers.append([p, pier_ekf.transition_covariance, t, np.zeros(pier_ekf.initial_state_mean.shape)])
    else:
    ...
    if len(tracked_person) == 0: 
        for t, p in cur_person:
            status = list(p)
            status.extend([0,0])
            tracked_person.append([np.array(status).flatten(), person_ekf.transition_covariance, t])
    else:
        tracked_person = update_person_track(tracked_person, cur_person, person_ekf)
